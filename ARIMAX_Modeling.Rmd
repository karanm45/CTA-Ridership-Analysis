...
ts_bus <- msts(df$bus, seasonal.periods = c(7, 365.25))

# Create Fourier terms for weekly and yearly seasonality for the entire dataset
fourier_terms_bus <- fourier(ts_bus, K = c(K_weekly, K_yearly))
...
The ARIMAX forecasts are much narrower than the actual boardings, but the seasonality is captured well by the fourier series.

```{r}
library(xts)
pd <- periodogram(train_set$bus)
```
...
ts_rail <- msts(df$rail_boardings, seasonal.periods = c(7, 365.25))

fourier_terms_rail <- fourier(ts_rail, K = c(K_weekly, K_yearly))
...
tf <- stats::filter(1*(seq(1:(length(train_set$rail) + steps.ahead ))==6708), filter=first.model.x$coef['pulse_train-AR1'] , method='recursive',side =1) * (first.model.x$coef['pulse_train-MA0'])
...
ts_tot <- msts(df$total_rides, seasonal.periods = c(7, 365.25))

fourier_terms_tot <- fourier(ts_tot, K = c(K_weekly, K_yearly))
...
# Calculate Coverage 
tolerance <- 0.3  
coverage_value <- mean(abs(tot_plot_data$Actual - tot_plot_data$Forecasted) / rail_plot_data$Actual <= tolerance) * 100
